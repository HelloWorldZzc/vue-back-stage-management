...
<!-- 卡片视图区域 -->
  <el-card>
    ...
    <tree-table class="treeTable" :data="catelist" :columns="columns" :selection-type="false" :expand-type="false" show-index index-text="#" border>
      <!-- 是否有效的作用栏（有点鸡肋...不是有效的数据服务器也不会返回给你，因此本列一定都是有效的数据），使用了自定义插槽 -->
...
<!-- 编辑商品分类的dialog 只能修改名字，等级修改之类的太麻烦了，并且毫无意义的事情 -->
  ...
  methods:{
    ...
    addCate() {
      this.$refs.addCateFormRef.validate(async valid => {
        if (!valid){
          return this.$message.error("请检查相关信息后再进行添加")
        }
   ...
   async removeCateById(id){
    ...
    editCate(){
      this.$refs.editCateFormRef.validate(async valid => {
        if (!valid) {
          return this.$message.error("请检查数据后再做添加")
        }
        // 发起修改用户信息的数据请求
